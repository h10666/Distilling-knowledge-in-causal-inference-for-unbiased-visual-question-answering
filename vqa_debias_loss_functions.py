...
class LearnedMixinWithTL(DebiasLossFn):
    def __init__(self, w, smooth=True, smooth_init=-1, constant_smooth=0.0):
        """
        :param w: Weight of the entropy penalty
        :param smooth: Add a learned sigmoid(a) factor to the bias to smooth it
        ...
        :param constant_smooth: Constant to add to the bias to smooth it
        """
        super(LearnedMixinWithTL, self).__init__()
        self.w = w
        self.smooth_init = smooth_init
        self.constant_smooth = constant_smooth
        self.bias_lin = torch.nn.Linear(1024, 1)
        self.bias_lin_t = torch.nn.Linear(1024, 1)
        self.smooth = smooth
        if self.smooth:
            self.smooth_param = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
            self.smooth_param_tl = torch.nn.Parameter(
                torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
        else:
            self.smooth_param = None

    def forward(self, hidden, logits, bias, labels, teacher_lables):
        factor = self.bias_lin.forward(hidden)  # [batch, 1]
        factor = F.softplus(factor)

        tl_factor = self.bias_lin_t.forward(hidden)
        tl_factor = F.softplus(tl_factor)

        # bias = teacher_lables * bias
        bias = torch.stack([bias, 1 - bias], 2)  # [batch, n_answers, 2]

        tl_bias = torch.stack([teacher_lables, 1 - teacher_lables], 2)
        ...
        if self.smooth:
            soften_factor = F.sigmoid(self.smooth_param)
            bias = bias + soften_factor.unsqueeze(1)
            soften_factor_tl = F.sigmoid(self.smooth_param_tl)
            tl_bias = tl_bias + soften_factor_tl.unsqueeze(1)

        bias = torch.log(bias)  # Convert to logspace
        tl_factor = torch.log(tl_factor)

        # Scale by the factor
        # [batch, n_answers, 2] * [batch, 1, 1] -> [batch, n_answers, 2]
        bias = bias * factor.unsqueeze(1)
        tl_bias = tl_bias * tl_factor.unsqueeze(1)

        log_prob, log_one_minus_prob = convert_sigmoid_logits_to_binary_logprobs(logits)
        log_probs = torch.stack([log_prob, log_one_minus_prob], 2)

        # Add the bias in
        logits = tl_bias + bias + log_probs
...
class BilinearLearnedMixin(DebiasLossFn):
    def __init__(self, w, smooth=True, smooth_init=-1, constant_smooth=0.0, rank=6):
        """
        :param w: Weight of the entropy penalty
        :param smooth: Add a learned sigmoid(a) factor to the bias to smooth it
        :param smooth_init: How to initialize `a`
        :param constant_smooth: Constant to add to the bias to smooth it
        """
        super(BilinearLearnedMixin, self).__init__()
        self.w = w
        self.smooth_init = smooth_init
        self.constant_smooth = constant_smooth
        self.bias_lin = torch.nn.Linear(1024, 1)
        ...
        self.bias_weights = nn.Linear(2274, 2274, bias=False)
        self.logits_weights = nn.Linear(2274, 2274, bias=False)

    def forward(self, hidden, logits, bias, labels):
        factor = self.bias_lin.forward(hidden)  # [batch, 1]
        factor = F.softplus(factor)
        logits = self.logits_weights(logits)
        bias = self.bias_weights(bias)
...
class LearnedMixin_vb_bias(DebiasLossFn):
    def __init__(self, w, smooth=True, smooth_init=-1, constant_smooth=0.0):
        """
        :param w: Weight of the entropy penalty
        :param smooth: Add a learned sigmoid(a) factor to the bias to smooth it
        ...
        :param constant_smooth: Constant to add to the bias to smooth it
        """
        super(LearnedMixin_vb_bias, self).__init__()
        self.w = w
        self.smooth_init = smooth_init
        self.constant_smooth = constant_smooth
        self.q_bias_lin = torch.nn.Linear(1024, 1)
        self.v_bias_lin = torch.nn.Linear(1024, 1)
        self.smooth = smooth
        if self.smooth:
            self.q_smooth_param = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
            self.v_smooth_param = torch.nn.Parameter(
                torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
        else:
            self.q_smooth_param = None
            self.v_smooth_param = None
        self.bceloss = BCELoss(reduction='none')

    def forward(self, hidden, logits, q_bias, v_bias, v_logits, labels):
        q_factor = self.q_bias_lin.forward(hidden)  # [batch, 1]
        q_factor = F.softplus(q_factor)

        q_bias = torch.stack([q_bias, 1 - q_bias], 2)  # [batch, n_answers, 2]

        # Smooth
        q_bias += self.constant_smooth
        if self.smooth:
            q_soften_factor = F.sigmoid(self.q_smooth_param)
            q_bias = q_bias + q_soften_factor.unsqueeze(1)

        q_bias = torch.log(q_bias)  # Convert to logspace

        # Scale by the factor
        # [batch, n_answers, 2] * [batch, 1, 1] -> [batch, n_answers, 2]
        q_bias = q_bias * q_factor.unsqueeze(1)

        log_prob, log_one_minus_prob = convert_sigmoid_logits_to_binary_logprobs(logits)
        log_probs = torch.stack([log_prob, log_one_minus_prob], 2)

        # Add the bias in
        logits = q_bias + log_probs
        ...
        # Re-normalized version of the bias
        q_bias_norm = elementwise_logsumexp(q_bias[:, :, 0], q_bias[:, :, 1])
        q_bias_logprob = q_bias - q_bias_norm.unsqueeze(2)

        # Compute and add the entropy penalty
        q_entropy = -(torch.exp(q_bias_logprob) * q_bias_logprob).sum(2).mean()
        return loss + self.w * q_entropy
