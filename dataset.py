...
def _create_entry(img_idx, question, answer, coco_split):
    answer.pop('image_id')
...
def _load_dataset(dataroot, name, img_id2val, cp=False):
    ...
    for question, answer in zip(questions, answers):
        ...
        if cp:
            ...
            coco_split = 'train2014' if name == "train" else 'val2014'

        entries.append(_create_entry(img_idx, question, answer, coco_split))
    return entries
...
class VQAFeatureDataset(Dataset):
    ...
    def get_item_by_queid(self, queid):
        entry = 0
        for item in self.entries:
            if item['question_id'] == queid:
                entry = item
                break
        path_rcnn = f"data/trainval_features/COCO_{entry['coco_split']}_{str(entry['image_id']).zfill(12)}.jpg.pth"
        img_path = f"data/trainval_features/COCO_{entry['coco_split']}_{str(entry['image_id']).zfill(12)}.jpg"
        item_rcnn = torch.load(path_rcnn)
        features = item_rcnn['pooled_feat']
        rois = item_rcnn['rois']
        ...
        if "bias" in entry:
            return features, question, target, entry["bias"], img_path, rois
        else:
            return features, question, target, 0, path_rcnn
